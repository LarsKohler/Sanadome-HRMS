  ...
  return (
    <>
      ...
      >
              ...
              return (
                <div key={section.label} className="select-none">
                  <button 
                    ...
                    className={`flex items-center justify-between w-full text-xs font-bold uppercase tracking-wider mb-2 px-3 py-2 rounded-lg transition-colors ${isExpanded ? 'text-slate-800 dark:text-slate-200 bg-slate-50 dark:bg-slate-800' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                  >
                    {section.label}
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <item.icon size={18} className={isActive ? 'text-teal-400' : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300'} />
                              <span className="font-bold text-sm">{item.label}</span>
                            </div>
                            {item.badge && (
                              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${isActive ? 'bg-white/20 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'}`}>
                                {item.badge}
                              </span>
                            )}
                          </button>
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 space-y-4">
            {onLogout && (
              <button 
                onClick={onLogout}
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-500 dark:text-slate-400 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 transition-all font-bold text-sm group"
              >
                <LogOut size={18} className="text-slate-400 dark:text-slate-500 group-hover:text-red-500 dark:group-hover:text-red-400" />
                Uitloggen
              </button>
            )}
            <div className="flex items-center justify-between px-2 text-[10px] font-bold text-slate-400 dark:text-slate-600 uppercase tracking-widest">
