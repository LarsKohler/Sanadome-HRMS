                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="mt-3 flex flex-wrap gap-2">
                                        {order.items.map((item, idx) => (
                                            <span key={idx} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-700">
                                                <span className="font-bold bg-white px-1.5 rounded-md border border-slate-200 shadow-sm">{item.quantity}</span>
                                                <span className="truncate max-w-[150px]" title={item.name}>{item.name}</span>
                                            </span>
                                        ))}
                                    </div>
                                    {order.notes && (
                                        <div className="text-xs text-amber-600 mt-2 italic flex items-center gap-1">
                            ...
                            <div key={order.id} className="bg-slate-50 rounded-xl border border-slate-200 p-4 flex justify-between items-center">
                                <div>
                                    ...
                                    <div className="mt-2 flex flex-wrap gap-2">
                                         {order.items.map((item, idx) => (
                                            <span key={idx} className="inline-flex items-center gap-1 text-[10px] text-slate-500 bg-white border border-slate-200 px-2 py-1 rounded">
