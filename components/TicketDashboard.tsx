...
import { Ticket, Search, Filter, CheckCircle2, Circle, Clock, AlertTriangle, MessageSquare, ChevronRight, RefreshCw, Lightbulb, Bug, Wrench, MapPin, Send, Plus, User, Paperclip, Lock, Globe, Calendar, Tag, X, Trash2 } from 'lucide-react';
...
const TicketDashboard: React.FC<TicketDashboardProps> = ({ onShowToast, currentUser, onAddNotification, onOpenFeedbackModal }) => {
    ...
    const handleSendMessage = async () => {
        ...
        // Notify if public reply
        if (!isInternalNote && onAddNotification && updatedTicket.submittedById !== currentUser.id) {
             const notif: Notification = {
                id: Math.random().toString(36).substr(2, 9),
                recipientId: updatedTicket.submittedById,
                senderName: currentUser.name,
                type: 'Ticket',
                title: `Nieuwe reactie: ${updatedTicket.title}`,
                message: `${currentUser.name} heeft gereageerd op je ticket.`,
                date: 'Zojuist',
                read: false,
    ...
    const handleStatusChange = async (newStatus: TicketStatus) => {
        ...
        // Notify user
        if (onAddNotification && updatedTicket.submittedById !== currentUser.id) {
             const notif: Notification = {
                id: Math.random().toString(36).substr(2, 9),
                recipientId: updatedTicket.submittedById,
                senderName: 'System',
                type: 'Ticket',
                title: `Status update: ${updatedTicket.title}`,
                message: `De status is gewijzigd naar: ${newStatus}`,
                date: 'Zojuist',
                read: false,
                targetView: ViewState.TICKETS,
                isPinned: false
    ...
    const renderMessage = (msg: TicketMessage) => {
        ...
        if (msg.type === 'internal' && !isManager) return null; // Double check security
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-64px)] bg-slate-50 overflow-hidden">
            ...
            <div className="flex flex-1 overflow-hidden max-w-[2400px] mx-auto w-full p-4 gap-4">
                ...
                <div className={`w-full md:w-1/3 lg:w-1/4 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col ${selectedTicketId ? 'hidden md:flex' : 'flex'}`}>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-2">
                        {!isManager && (
                            <button 
                                ...
                                className="w-full py-3 border-2 border-dashed border-slate-200 rounded-xl text-slate-500 font-bold text-sm hover:border-purple-400 hover:text-purple-600 hover:bg-purple-50 transition-all flex items-center justify-center gap-2 mb-2"
                            >
                ...
                {selectedTicket ? (
                    <div className={`flex-1 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row overflow-hidden ${!selectedTicketId ? 'hidden md:flex' : 'flex'}`}>
