...
const DebtControlPage: React.FC<DebtControlPageProps> = ({ currentUser, onShowToast }) => {
  ...
  // ... (Address enrichment & File upload logic skipped for brevity - no changes) ...
      ...
      try {
          ...
          for (let i = 1; i < data.length; i++) {
              ...
              if (balance > 0) {
                  ...
                  if (existingNumbers.has(reservationNumber)) { skippedCount++; continue; }
                  ...
                  if (matchZipFirst) { zipToEnrich = matchZipFirst[1]; numberToEnrich = matchZipFirst[2]; } 
                  else if (matchZipLast) { numberToEnrich = matchZipLast[1]; zipToEnrich = matchZipLast[2]; }
                  ...
                  if (zipToEnrich && numberToEnrich) {
                      ...
                      if (enriched) {
                          ...
                          if (address !== newAddress) { address = newAddress; isEnriched = true; enrichedCount++; }
                      }
                  }
  ...
  // ... (parseAddress, handleOpenDetail, handleOpenCreate, handleSaveDetails, handleAddNoteInDetail, handleIgnoreAddressWarning, openDateEdit, handleDateSave, openWikModal, generateWIKLetter logic omitted for brevity - unchanged) ...
  const parseAddress = (rawAddr: string) => {
      let street = ''; let number = ''; let zip = ''; let city = ''; let country = 'Nederland';
      if (!rawAddr) return { street, number, zip, city, country };
      const cleanAddr = rawAddr.trim();
      const zipRegex = /\b\d{4}\s?[A-Za-z]{2}\b|\b\d{4,5}\b/;
      let parts = cleanAddr.split(',').map(s => s.trim()).filter(s => s.length > 0);
      if (parts.length > 2) {
          const lastPart = parts[parts.length - 1];
          if (!/\d/.test(lastPart)) { country = parts.pop()!; }
      }
      for (let i = parts.length - 1; i >= 0; i--) {
          ...
          if (match) {
              ...
              zip = rawZip.replace(/\s+/g, '');
              if (zip.length === 6 && /[A-Z]/.test(zip)) { zip = zip.slice(0, 4) + ' ' + zip.slice(4); }
              const remainder = part.replace(rawZip, '').trim();
              if (remainder.length > 0) { city = remainder.replace(/^[-,\s]+|[-,\s]+$/g, ''); }
              if (!city && i > 0) {
                   const prev = parts[i-1];
                   if (!/\d/.test(prev)) { city = prev; parts.splice(i-1, 1); i--; }
              }
              if (!city && i < parts.length - 1) { city = parts[i+1]; parts.splice(i+1, 1); }
              parts.splice(i, 1); break;
          }
      }
      if (!city && parts.length > 1) {
          const last = parts[parts.length - 1];
          if (!/\d/.test(last)) { city = parts.pop()!; }
      }
      let fullStreet = parts.join(' ').trim();
      const numberMatch = fullStreet.match(/(\d+[\w\s-]*)$/);
      if (numberMatch) {
          number = numberMatch[0].trim();
          street = fullStreet.substring(0, numberMatch.index).trim();
      } else {
          const startNumMatch = fullStreet.match(/^(\d+)\s+(.*)/);
          if (startNumMatch) { number = startNumMatch[1]; street = startNumMatch[2]; } else { street = fullStreet; }
      }
  ...
  const handleOpenDetail = (debtor: Debtor) => {
      setDetailDebtor(debtor); setIsCreating(false); setIsAddressParseError(false);
  ...
  const handleOpenCreate = () => {
      const dummyDebtor: Debtor = { id: 'new', reservationNumber: '', firstName: '', lastName: '', address: '', amount: 0, status: 'New', statusDate: new Date().toISOString(), lastUpdated: new Date().toISOString(), importedAt: new Date().toLocaleDateString('nl-NL') };
      setDetailDebtor(dummyDebtor); setIsCreating(true); setIsAddressParseError(false);
      setDetailForm({ reservationNumber: '', firstName: '', lastName: '', email: '', phone: '', addressStreet: '', addressNumber: '', addressZip: '', addressCity: '', addressCountry: 'Nederland', amount: 0, status: 'New', statusDate: new Date().toISOString().split('T')[0], cashlistReason: '', correctionReason: '' });
      setNewDetailNote('');
  };

  const handleSaveDetails = async () => {
      if (!detailDebtor) return;
      if (detailForm.status === 'Cashlist' && !detailForm.cashlistReason.trim()) { onShowToast("Reden voor Cashlist is verplicht."); return; }
      if (detailForm.status === 'Correction' && !detailForm.correctionReason.trim()) { onShowToast("Reden voor Correctie is verplicht."); return; }
      if (isCreating) {
      ...
      if (isCreating) {
          const newDebtor: Debtor = {
              ...
              notes: newDetailNote.trim() ? [{ id: Math.random().toString(36).substr(2, 9), content: newDetailNote, date: new Date().toISOString(), author: currentUser.name }] : []
          };
          const updatedList = [newDebtor, ...debtors];
          setDebtors(sortDebtors(updatedList));
          await api.saveDebtors(updatedList);
          setDetailDebtor(null); setIsCreating(false); onShowToast("Nieuw dossier aangemaakt.");
      } else {
          ...
          if (detailForm.status !== detailDebtor.status) {
              ...
              currentNotes.push({ id: Math.random().toString(36).substr(2, 9), content: noteContent, date: new Date().toISOString(), author: currentUser.name });
          }
  ...
  const handleAddNoteInDetail = async () => {
      if (!detailDebtor || !newDetailNote.trim() || isCreating) return;
      const newNote: DebtorNote = { id: Math.random().toString(36).substr(2, 9), content: newDetailNote, date: new Date().toISOString(), author: currentUser.name };
      const updatedDebtor = { ...detailDebtor, notes: [...(detailDebtor.notes || []), newNote] };
      const updatedList = debtors.map(d => d.id === updatedDebtor.id ? updatedDebtor : d);
      setDebtors(sortDebtors(updatedList)); setDetailDebtor(updatedDebtor); setNewDetailNote('');
      await api.saveDebtors(updatedList); onShowToast("Notitie toegevoegd");
  };

  const handleIgnoreAddressWarning = (id: string) => { setIgnoredAddressWarnings(prev => new Set(prev).add(id)); };
  ...
  const handleDateSave = async (e: React.FormEvent) => {
      ...
      const updatedList = debtors.map(d => {
          if (d.id === dateEditTarget.id) { return { ...d, statusDate: new Date(newDateValue).toISOString() }; }
          return d;
      });
      setDebtors(sortDebtors(updatedList)); await api.saveDebtors(updatedList); setDateEditTarget(null); onShowToast("Datum succesvol aangepast");
  };

  const openWikModal = (debtor: Debtor) => {
      setWikTarget(debtor); setWikDateInput('');
      const addressLower = (debtor.address || '').toLowerCase();
      if (addressLower.includes('deutschland') || addressLower.includes('germany')) { setWikLanguage('de'); } 
      else if (addressLower.includes('nederland') || addressLower.includes('netherlands')) { setWikLanguage('nl'); } 
      else { setWikLanguage('en'); }
  };
  ...
  const generateWIKLetter = () => {
      ...
      let line1 = ''; let line2 = ''; let line3 = 'NEDERLAND';
      const parts = addrString.split(',').map(s => s.trim());
      if (parts.length >= 3) { line1 = parts[0]; line2 = parts[1]; line3 = parts[2]; } 
      else if (parts.length === 2) { line1 = parts[0]; line2 = parts[1]; } 
      else { line1 = addrString; }
      const translations = {
          nl: { subject: "Betalingsherinnering – Laatste aanmaning", salutation: `Beste ${wikTarget.lastName},`, body: `<p>Hierbij herinneren wij u aan de openstaande factuur met reserveringsnummer <strong>${wikTarget.reservationNumber}</strong> van <strong>${formattedDateInput}</strong> met een bedrag van <strong>€${amountFormatted}</strong>.</p><p>Helaas hebben wij, ondanks meerdere herinneringen, tot op heden nog geen betaling van u mogen ontvangen. Wij verzoeken u vriendelijk het verschuldigde bedrag binnen 14 dagen over te maken naar ons rekeningnummer <strong>NL52 RABO 0181 6526 68</strong>, ten name van Sanadome Hotel & Spa Nijmegen, onder vermelding van het reserveringsnummer.</p><p>Wij wijzen u erop dat wij bij uitblijven van tijdige betaling genoodzaakt zijn de vordering over te dragen aan een externe incassopartij. In dat geval worden incassokosten en wettelijke rente in rekening gebracht, conform de geldende wettelijke regelingen.</p><p>Mocht u inmiddels wel betaald hebben, dan kunt u deze aanmaning als niet verzonden beschouwen.</p><p>Indien u vragen, of opmerkingen met betrekking tot deze factuur heeft, kunt u ten allertijden contact opnemen met ons via de contactgegevens onderstaand deze brief.</p><p>Wij vertrouwen erop dat u de betaling alsnog tijdig zult voldoen en hopen hiermee verdere incassomaatregelen te voorkomen.</p>`, closing: "Met hartelijke groet," },
          en: { subject: "Payment Reminder – Final Notice", salutation: `Dear ${wikTarget.lastName},`, body: `<p>We are writing to remind you of the outstanding invoice with reservation number <strong>${wikTarget.reservationNumber}</strong> dated <strong>${formattedDateInput}</strong> with an amount of <strong>€${amountFormatted}</strong>.</p><p>Unfortunately, despite previous reminders, we have not yet received payment from you. We kindly request that you transfer the amount due within 14 days to our bank account <strong>NL52 RABO 0181 6526 68</strong>, in the name of Sanadome Hotel & Spa Nijmegen, stating the reservation number.</p><p>Please be advised that if payment is not made on time, we will be forced to hand over the claim to an external collection agency. In that case, collection costs and statutory interest will be charged in accordance with applicable legal regulations.</p><p>If you have already paid, please disregard this notice.</p><p>If you have any questions or comments regarding this invoice, please feel free to contact us via the contact details below.</p><p>We trust that you will settle the payment promptly to avoid further collection measures.</p>`, closing: "With kind regards," },
          de: { subject: "Zahlungserinnerung – Letzte Mahnung", salutation: `Sehr geehrte(r) ${wikTarget.lastName},`, body: `<p>hiermit erinnern wir Sie an die offene Rechnung mit der Reservierungsnummer <strong>${wikTarget.reservationNumber}</strong> vom <strong>${formattedDateInput}</strong> über einen Betrag von <strong>€${amountFormatted}</strong>.</p><p>Leider haben wir trotz mehrfacher Erinnerungen bis heute keinen Zahlungseingang von Ihnen feststellen können. Wir bitten Sie freundlich, den fälligen Betrag innerhalb von 14 Tagen auf unser Konto <strong>NL52 RABO 0181 6526 68</strong>, lautend auf Sanadome Hotel & Spa Nijmegen, unter Angabe der Reservierungsnummer zu überweisen.</p><p>Wir weisen Sie darauf hin, dass wir uns bei Ausbleiben einer fristgerechten Zahlung gezwungen sehen, die Forderung an ein externes Inkassobüro zu übergeben. In diesem Fall werden Inkassokosten und gesetzliche Zinsen gemäß den geltenden gesetzlichen Bestimmungen berechnet.</p><p>Sollten Sie die Zahlung bereits geleistet haben, betrachten Sie dieses Schreiben bitte als gegenstandslos.</p><p>Sollten Sie Fragen oder Anmerkungen zu dieser Rechnung haben, können Sie uns jederzeit über die untenstehenden Kontaktdaten erreichen.</p><p>Wir vertrauen darauf, dass Sie die Zahlung nun zeitnah begleichen, um weitere Inkassomaßnahmen zu vermeiden.</p>`, closing: "Mit freundlichen Grüßen," }
      };
      const t = translations[wikLanguage];
      const letterContent = `<html><head><title>WIK Brief - ${wikTarget.lastName}</title><style>body { font-family: 'Calibri', 'Segoe UI', sans-serif; padding: 40px; padding-top: 60mm; font-size: 11pt; line-height: 1.3; color: #000; } .header { display: flex; justify-content: space-between; margin-bottom: 60px; margin-top: -40px; } .recipient { width: 50%; line-height: 1.4; } .sender { width: 35%; text-align: left; font-size: 11pt; margin-left: auto; line-height: 1.4; } .sender-bold { font-weight: bold; } .meta { margin-bottom: 40px; } .subject { font-weight: bold; text-decoration: underline; margin-bottom: 20px; } .content { margin-bottom: 40px; text-align: justify; } .signature { margin-top: 0px; } .signature strong { display: block; margin-top: 0px; } @media print { @page { margin: 2cm; margin-top: 0; } body { padding: 0; padding-top: 60mm; } }</style></head><body><div class="header"><div class="recipient"><strong>${wikTarget.firstName} ${wikTarget.lastName}</strong><br>${line1}<br>${line2}<br><span style="font-weight: bold;">${line3.toUpperCase()}</span></div><div class="sender"><span class="sender-bold">Sanadome Hotel & Spa</span><br>Weg door Jonkerbos 90<br>6532 SZ Nijmegen</div></div><div class="meta">Nijmegen, ${currentDate}</div><div class="subject">${t.subject}</div><div class="content"><p>${t.salutation}</p>${t.body}</div><div class="signature">${t.closing}<br><strong>${currentUser.name} | ${currentUser.role}</strong></div></body></html>`;
      const printWindow = window.open('', '_blank');
      if (printWindow) { printWindow.document.write(letterContent); printWindow.document.close(); printWindow.focus(); setTimeout(() => { printWindow.print(); printWindow.close(); }, 250); }
      setWikTarget(null);
  };

  // --- UPDATED TOTAL CALCULATIONS ---
  
  // Goal 1 & 3: Filtered list + Exclude Cashlist, Paid, Correction
  const totalVisibleDebt = filteredDebtors
      .filter(d => d.status !== 'Paid' && d.status !== 'Correction' && d.status !== 'Cashlist')
      .reduce((acc, curr) => acc + curr.amount, 0);

  // Goal 2: Total of selected items
  const totalSelectedDebt = debtors
  ...
  const getModalStatusOptions = () => {
      ...
      const isSkipping = (target: DebtorStatus) => {
          ...
          if (target === 'Paid' || target === 'Correction' || target === 'Cashlist') return false; 
      ...
      return (
          <div className="grid grid-cols-2 gap-4">
              <StatusOptionCard status="New" label="Nieuw" description="Nog geen actie ondernomen." colorClass="bg-blue-50/50 border-blue-200 text-blue-700 hover:border-blue-400" onClick={() => handleStatusSelect('New')} recommended={false} dimmed={false} />
              <StatusOptionCard status="1st Reminder" label="1e Herinnering" description="Eerste mail/brief verstuurd (+7 dagen)." colorClass="bg-amber-50/50 border-amber-200 text-amber-700 hover:border-amber-400" onClick={() => handleStatusSelect('1st Reminder')} recommended={nextStep === '1st Reminder'} dimmed={isSkipping('1st Reminder')} />
              <StatusOptionCard status="2nd Reminder" label="2e Herinnering" description="Tweede waarschuwing (+7 dagen)." colorClass="bg-orange-50/50 border-orange-200 text-orange-700 hover:border-orange-400" onClick={() => handleStatusSelect('2nd Reminder')} recommended={nextStep === '2nd Reminder'} dimmed={isSkipping('2nd Reminder')} />
              <StatusOptionCard status="Final Notice" label="Aanmaning" description="Laatste waarschuwing (+14 dagen)." colorClass="bg-red-50/50 border-red-200 text-red-700 hover:border-red-400" onClick={() => handleStatusSelect('Final Notice')} recommended={nextStep === 'Final Notice'} dimmed={isSkipping('Final Notice')} />
              <StatusOptionCard status="Paid" label="Betaald" description="Dossier succesvol afgerond." colorClass="bg-green-50/50 border-green-200 text-green-700 hover:border-green-400" onClick={() => handleStatusSelect('Paid')} recommended={false} dimmed={false} />
              <StatusOptionCard status="Correction" label="Correctie" description="Administratief gecorrigeerd (0)." colorClass="bg-slate-100 border-slate-300 text-slate-700 hover:border-slate-500" onClick={() => handleStatusSelect('Correction')} recommended={false} dimmed={false} />
              <StatusOptionCard status="Cashlist" label="Cashlist" description="Alleen vooraf betalen." colorClass="bg-purple-50/50 border-purple-200 text-purple-800 hover:border-purple-400" onClick={() => handleStatusSelect('Cashlist')} recommended={nextStep === 'Cashlist'} dimmed={false} />
          </div>
      );
  };
    ...
    <div className="p-6 lg:p-10 w-full max-w-[2400px] mx-auto animate-in fade-in duration-500 min-h-[calc(100vh-80px)] pb-24 bg-slate-50">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
        ...
        <div className="flex gap-3">
             <button onClick={handleOpenCreate} className="flex items-center gap-2 px-6 py-3.5 bg-white border border-slate-200 text-slate-700 font-bold rounded-xl shadow-sm hover:bg-slate-50 transition-all hover:shadow-md"><Plus size={20}/> Nieuw Dossier</button>
             <input type="file" ref={fileInputRef} accept=".csv, .xlsx, .xls" className="hidden" onChange={handleFileUpload} />
             <button onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="flex items-center gap-3 px-6 py-3.5 bg-slate-900 text-white font-bold rounded-xl shadow-lg hover:bg-slate-800 transition-all hover:-translate-y-0.5 disabled:opacity-70 hover:shadow-xl">{isUploading ? <RefreshCw className="animate-spin" size={20}/> : <Upload size={20}/>} {isUploading ? 'Verwerken...' : 'Importeer Rapportage'}</button>
        </div>
      </div>
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-white to-slate-50 p-6 rounded-2xl border border-slate-200 shadow-sm group hover:shadow-md transition-all">
              ...
              <div className="relative z-10">
                  ...
                  <div className="text-3xl xl:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-slate-900 to-slate-700">
                      € {totalVisibleDebt.toLocaleString('nl-NL', {minimumFractionDigits: 2})}
                  </div>
                  <div className="text-xs text-slate-400 mt-1 font-medium">Enkel actieve dossiers</div>
              </div>
          </div>
          ...
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-2 py-2 rounded-2xl shadow-2xl z-40 flex items-center gap-2 animate-in slide-in-from-bottom-10 duration-300 border border-slate-700/50 backdrop-blur-xl bg-opacity-95">
              <div className="flex items-center gap-3 font-bold px-4 py-2">
                  ...
                  <div className="flex flex-col">
                      <span className="text-sm leading-none">Geselecteerd</span>
                      <span className="text-xs text-slate-400 font-normal">
                          € {totalSelectedDebt.toLocaleString('nl-NL', {minimumFractionDigits: 2})}
                      </span>
                  </div>
              </div>
